...
import { Download, X, ExternalLink, FileText, ArrowLeft, Eye, Database, ChevronDown } from 'lucide-react';
            ...
            <div className="lg:col-span-1 space-y-4 overflow-y-auto">
              <Tabs defaultValue="info" className="w-full">
                ...
                <TabsContent value="data">
                  <ExtractedDataDisplay document={document} extractedData={extractedData} />
                </TabsContent>
              </Tabs>
            </div>
          </div>
...
// Component to display extracted data from documents
interface ExtractedDataDisplayProps {
  document: ObraDocument;
  extractedData: any;
}

function ExtractedDataDisplay({ document, extractedData }: ExtractedDataDisplayProps) {
  const [isOcrExpanded, setIsOcrExpanded] = React.useState(false);
  ...
  return (
    <div className="space-y-4">
        ...
        <Collapsible open={isOcrExpanded} onOpenChange={setIsOcrExpanded}>
          <CollapsibleTrigger asChild>
            <Button variant="outline" className="w-full justify-between">
              ...
              <ChevronDown className={`h-4 w-4 transition-transform ${isOcrExpanded ? 'rotate-180' : ''}`} />
            </Button>
          </CollapsibleTrigger>
      ...
      {extractedData && extractedData.extracted_data && Object.keys(extractedData.extracted_data).length > 0 && (
            ...
            <Button variant="outline" className="w-full justify-between">
              <span className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="ml-1">
                  {Object.keys(extractedData.extracted_data).length} campos
                </Badge>
              </span>
              <ChevronDown className={`h-4 w-4 transition-transform ${isDataExpanded ? 'rotate-180' : ''}`} />
            </Button>
          </CollapsibleTrigger>
          <CollapsibleContent className="mt-2">
            <div className="space-y-2">
              ...
              <div className="mt-3 pt-3 border-t border-border">
                <div className="flex justify-between items-center text-xs text-muted-foreground">
                  ...
                  <Badge 
                    variant={
                      extractedData.extraction_confidence > 0.8 ? 'default' : 
                      extractedData.extraction_confidence > 0.5 ? 'secondary' : 
                      'destructive'
                    }
                  >
                ...
                <div className="flex justify-between items-center text-xs text-muted-foreground mt-1">
                  ...
                  <span>{extractedData.field_count || 0}</span>
                </div>
              </div>
            </div>
          </CollapsibleContent>
      ...
      {!document.ocr_content && (!extractedData || !extractedData.extracted_data || Object.keys(extractedData.extracted_data).length === 0) && (
        <div className="text-center py-8 text-muted-foreground">
          <Database className="h-8 w-8 mx-auto mb-2 opacity-50" />
          <p className="text-sm">No hay datos extraídos disponibles</p>
          <p className="text-xs mt-1">
            {document.type.startsWith('image/') || document.type === 'application/pdf' 
              ? 'Puede que este documento no haya sido procesado con OCR' 
              : 'Este tipo de archivo no admite extracción de datos'
            }
          </p>
        </div>
      )}

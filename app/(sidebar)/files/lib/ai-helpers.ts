...
async function convertImageUrlToBase64(imageUrl: string, fileType?: string): Promise<string> {
  try {
    ...
    console.log('[AI] Successfully converted image to base64, size:', Math.round(base64.length / 1024), 'KB');
...
export async function extractStructuredFields(
  ...
  try {
    ...
    const fieldDescriptions = fieldDefinitions.map(field => {
      ...
      const pattern = field.extraction_pattern ? ` - Pattern: ${field.extraction_pattern}` : '';
...
export async function processDocumentWithAI(
  ...
  if (fileType.includes('image') || fileType.includes('png') || fileType.includes('jpg')) {
    try {
      ...
      console.error('[AI] OpenAI Vision failed, trying text extraction:', error);
    }
  }

...
export function DocumentTreeView({
  ...
  const buildTree = (): TreeNode[] => {
    ...
    // Add folders with their documents first
    ...
    // Add root-level documents (documents without folder) after folders
      ...
      return (
        <div key={node.id} className="space-y-1">
          <div className={cn("flex items-center gap-1", indentClass)}>
            ...
            <Link
              href={`/files?${buildQuery({ folder: node.id })}`}
              className={cn(
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Link
          href={`/files?${buildQuery({ folder: undefined })}`}
          className={cn(

...
import { Plus, Upload, X, FileText, Clock } from 'lucide-react';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        throw new Error(result.error || `Upload failed. ${result.data?.errors?.length || 0} errors occurred.`);
      }

      // Optimistic update: show success immediately
      const successMessage = `${files.length} archivo${files.length !== 1 ? 's' : ''} subido${files.length !== 1 ? 's' : ''} correctamente`;
      toast.success(successMessage);

      // Show background processing message
      if (folderHasExtraction) {
        toast.info('🤖 Procesando con IA y extrayendo campos en segundo plano...', {
          duration: 4000,
        });
      } else {
      ...
      if (result.data?.errors && result.data.errors.length > 0) {
        result.data.errors.forEach((error: any) => {
          toast.error(`Error con ${error.file}: ${error.error}`, {
            duration: 5000,
          });
        });
      }
